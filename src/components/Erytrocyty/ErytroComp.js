...
function ErytroComp (props) {
    ...
    const [selectedElement, setSelectedElement] = useState('--- nazwa nieprawidłowości ---');
    ...
    const numberObs = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X']
    ...
    const obsFormHandler = (e) => {
        ...
        setResult(
            <div>
            <OutcomeErytro key={nanoid()} selected={selectedObject(selectedElement)} data={obsVal}/>
            </div>
        )
    }
    ...
    return (
        <div>
    <fieldset className="erytro_element">
    ...
    <div className="pole_wrapper">
        ...
        <div className="name_wrapper">
            <select className="name_select" value={selectedElement} onChange={selectedHandler}>
        ...
        {/*Generacja pól do wprowadzania zarejestrowanych wartości */}
            <input key = {el} className="pole" type="number" min="0" max="99" placeholder="0" name={`${el}`} value={obsVal[`${el}`]} onChange={obsHandler}></input>
                </div>
            ))}
            <button type="submit" className="btn_erytro_oblicz">{'=>'}</button>
        {result}
        </div>
        </form>
    </div>
