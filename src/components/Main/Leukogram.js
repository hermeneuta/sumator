...
function Leukogram (props) {
  const names = [
    ...
    {
      name: 'PAL',
      description: 'neutrocyty pałeczkowate – neutrofile z jądrem pałeczkowatym',
      group: 'neut_not_count',
      count: 0,
      report: 'first',
      leu: true
    },
    {
      name: 'NEU',
      description: 'neutrocyty – granulocyty obojętnochłonne z jądrem podzielonym',
      group: 'neut',
      count: 0,
      report: 'first',
      leu: true
    ...
    {
      name: 'LGL% LIM',
      description: 'Odsetek LGL wśród wszystkich limfocytów (suma od LIM do LGL)',
      leu: false,
      hiden: true,
      report: 'other other5',
    },
    ...
    {
      name: 'INNE',
      description: 'inne kategorie komórek: mikromegakariocyty, komórki nowotworów niehematologicznych, mikroorganizmy',
      count: 0,
      leu: false,
      report: 'other other8',
      display: 'last_column',
    ...
    const onPrzelicz = () => {
        setShowReport(true);
        setReport(<ReportBList elements={elementsObs} onReturn={onReturn}/>)
    }
    ...
    return (
        ...
        <div className="back">
          ...
          {!showReport && <div> <ElementsList
            elements={names}
            detPass={detected} /> </div>}
